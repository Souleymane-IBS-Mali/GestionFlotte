...
$monform = new Form1($db);
	...
	if($action == "ajouter_maintenance"){
		...
		if(empty($type_maintenance))
			$message .= 'Le champ "TYPE MAINTENANCE" est oblogatoire<br>';
		...
		//Chargement de l'image
			if (isset($_FILES['facture']) && $_FILES['facture']['error'] == 0 && empty($message)) {
				...
				$identification = $obj_vehicule->reference_interne."_facture".$rowid_doc;
		...
		if(empty($message)){
			$piece = implode(',', $piece_remplace);
			$sql_insert = 'INSERT INTO '.MAIN_DB_PREFIX.'maintenance_vehicule (maintenance_reparation, commentaire, fk_vehicule, fk_type_maintenance, date_maintenance, travaux_effectue, fk_piece_remplacer, cout, date_fin, fk_user_creation';
			if($img)
				$sql_insert .= ', facture';
			$sql_insert .= ')';

			$sql_insert .= ' VALUES(1, "'.$description.'", '.$id_vehicule.', '.$type_maintenance.', "'.$date.'","'.$nature_traveaux.'", "'.$piece.'", '.$cout.', "'.$date_fin.'", '.$user->id;
			if($img)
				$sql_insert .= ', "'.$img.'"';
			...
			if($res_insert){
				$message = "Une maintenance enregistrée avec succès";
				header('Location: '.$_SERVER["PHP_SELF"].'?mainmenu=gestionflotte&leftmenu=listevehicule&id_vehicule='.$id_vehicule.'&message='.$message);
			}else{
	...
	if($action == "ajouter"){
			...
			print '<input type="hidden" name="action" value="ajouter_maintenance">';
			...
			print "<tr><td class='fieldrequired' style='width: 250px; padding-top: 10px;'>Date maintenance</td><td style='width: 200px; padding-top: 10px;'><input type='date' name='date_maintenance' id='date_maintenance' value='".GETPOST("date_maintenance")."'></td></tr>";

			print "<tr><td class='fieldrequired' style='width: 250px; padding-top: 10px;'>Date fin</td><td style='width: 200px; padding-top: 10px;'><input type='date' name='date_fin' id='date_fin' value='".GETPOST("date_fin")."'></td></tr>";

			print "<tr><td class='fieldrequired' style='width: 250px; padding-top: 10px;'>Type maintenance</td><td><select
			name='type_maintenance' id='type_maintenance'>";
			print "<option value='0'></option>";
			$sql_type_maintenance = "SELECT rowid, libelle FROM ".MAIN_DB_PREFIX."type_maintenance WHERE rowid != 1";
			$restype_maintenance = $db->query($sql_type_maintenance);
			if($restype_maintenance){
				$nb = $db->num_rows($restype_maintenance);
				$i =0;
				while ($i < $nb) {
					$obj_typ_cont = $db->fetch_object($restype_maintenance);
					if(!empty(GETPOST("type_maintenance", "int")) && GETPOST("type_maintenance", "int") == $obj_typ_cont->rowid)
		...
		$annee_rechercher = GETPOST('annee', 'int')?:date('Y');
			...
			print "<h3 >Maintenance en cours de <mark>".$annee_rechercher."</mark></h3>";
			print "<table class='tagtable liste' style='width:100%;'>";
			print "<tr class='liste_titre'><td style='padding: 10px;'>Description</td><td style='padding: 10px;'>Type maintenance</td><td style='padding: 10px;'>Date maintenance</td><td style='padding: 10px;'>Date fin</td><td style='padding: 10px;'>Traveaux effectués";
			...
			$sql_maintenance .= " WHERE mv.fk_type_maintenance != '1' AND mv.maintenance_reparation = 1 AND mv.fk_vehicule=".$id_vehicule." AND mv.date_fin >= '".$aujourdhui."'";
			if($annee_rechercher != date('Y'))
			...
			if($res_maintenance){
				...
				while ($obj_maintenance = $db->fetch_object($res_maintenance)) {
					$desc = img_picto('','maintenance', 'class="paddingright pictofixedwidth"').$obj_maintenance->commentaire;
					$type_maint = img_picto('','maintenance', 'class="paddingright pictofixedwidth"').'<a href="../creation_type_maintenance.php?mainmenu=gestionflotte&leftmenu=listevehicule&recherche_libelle='.$obj_maintenance->libelle.'">'.$obj_maintenance->libelle.'</a>';

					print "<tr><td style='padding: 10px;'>".$desc."</td><td style='padding: 10px;'>".$type_maint."</td><td style='padding: 10px;'>".$obj_maintenance->date_maintenance."</td><td style='padding: 10px;'>".$obj_maintenance->date_fin."</td>";
					...
					print "<td style='padding: 10px;'>".$outils."</td><td style='padding: 10px;'>".$img."</td><td style='padding: 10px;'>".$actl[1]."</td></tr>";
			...
			//les maintenances expiréss
			print "<br><div>";
			print "<h3 >Historique de maintenance de <mark>".$annee_rechercher."</mark></h3>";
			print "<table class='tagtable liste' style='width:100%;'>";
			print "<tr class='liste_titre'><td style='padding: 10px;'>Description</td><td style='padding: 10px;'>Type maintenance</td><td style='padding: 10px;'>Date maintenance</td><td style='padding: 10px;'>Date fin</td><td style='padding: 10px;'>Traveaux effectués";
			print "</td><td style='padding: 10px;'>Pièces remplacées</td><td style='padding: 10px;'>Facture</td><td style='padding: 10px;'>Statut</td></tr>";
			...
			$sql_maintenance = "SELECT mv.*, tm.rowid as tmrowid, tm.libelle FROM ".MAIN_DB_PREFIX."maintenance_vehicule as mv";
			$sql_maintenance .= " LEFT JOIN ".MAIN_DB_PREFIX."type_maintenance as tm on mv.fk_type_maintenance = tm.rowid";
			$sql_maintenance .= " WHERE mv.maintenance_reparation = 1 AND mv.fk_vehicule=".$id_vehicule." AND mv.date_fin <= '".$aujourdhui."'";
			if($annee_rechercher != date('Y'))
				$sql_maintenance .= " AND YEAR(date_maintenance) = ".$annee_rechercher;
			...
			if($res_maintenance){
				...
				while ($obj_maintenance = $db->fetch_object($res_maintenance)) {
					...
					print "<tr><td style='padding: 10px;'>".$type_maint."</td><td style='padding: 10px;'>".$obj_maintenance->date_maintenance."</td><td style='padding: 10px;'>".$obj_maintenance->date_fin."</td>";
					print affiche_long_texte('',  $obj_maintenance->travaux_effectue, 1, '', '', '', '', '', '');;
					$outils = "";
						if($obj_maintenance->fk_piece_remplacer != -1){
							$sql_outil = "SELECT nom FROM ".MAIN_DB_PREFIX."piece WHERE rowid IN (".$obj_maintenance->fk_piece_remplacer.") ORDER BY nom";
					...
					$img = '<a title="Visualiser" target="_blank" href="./facture_maintenance_reparation/'.$obj_maintenance->facture.'">'.$obj_maintenance->facture.'</a>';
					print "<td style='padding: 10px;'>".$outils."</td><td style='padding: 10px;'>".$img."</td><td style='padding: 10px;'>".$actl[0]."</td></tr>";
