...
$monform = new Form1($db);
	...
	if($action == "ajouter_maintenance"){
		...
		$kilometre = GETPOST('kilometre', 'alpha');

		if(empty($description))
			$message = 'Le champ "DESCRIPTION" est oblogatoire<br>';

		if(empty($kilometre))
			$message .= 'Le champ "PROCHAIN KILOMETRAGE" est oblogatoire<br>';
		...
		if(empty($message)){
			...
			$sql_insert = 'INSERT INTO '.MAIN_DB_PREFIX.'maintenance_vehicule (maintenance_reparation, prochain_kilometrage, commentaire, fk_vehicule, fk_type_maintenance, date_maintenance, travaux_effectue, fk_piece_remplacer, cout, fk_user_creation';
			if($img)
				$sql_insert .= ', facture';
			...
			$sql_insert .= ' VALUES(1, '.$kilometre.', "'.$description.'", '.$id_vehicule.', '.$type_maintenance.', "'.$date.'","'.$nature_traveaux.'", "'.$piece.'", '.$cout.', '.$user->id;
			if($img)
				$sql_insert .= ', "'.$img.'"';
...
print $db->error();
	...
	print dol_get_fiche_head($head, 'vidange', "", -1, '');
	...
	if($action == "ajouter"){
			...
			//Coût
			print "<tr><td class='fieldrequired' style='width: 250px; padding-top: 10px;'>Prochaine Kilométrage</td><td style='width: 200px; padding-top: 10px;'>
			<input type='number' step='any' id='kilometre' name='kilometre' value='".GETPOST('kilometre')."'/> Km";
			print "</td></tr>";
			...
			//date fin
			print "<tr><td style='width: 250px; padding-top: 10px;'>Date fin</td><td style='width: 200px; padding-top: 10px;'><input type='date' name='date_fin' id='date_fin' value='".GETPOST("date_fin")."'></td></tr>";
			...
			print "<h3 >Vidanges éffectués en <mark>".$annee_rechercher."</mark></h3>";
			print "<table class='tagtable liste' style='width:100%;'>";
			print "<tr class='liste_titre'><td style='padding: 10px;'>Description</td><td style='padding: 10px;'>Type maintenance</td><td style='padding: 10px;'>Date maintenance</td><td style='padding: 10px;'>Traveaux effectués";
			print "</td><td style='padding: 10px;'>Prochain Kilométrage</td><td style='padding: 10px;'>Pièces remplacées</td><td style='padding: 10px;'>Facture</td></tr>";
			...
			$sql_maintenance .= " WHERE mv.maintenance_reparation = 1 AND mv.fk_type_maintenance = '1' AND mv.fk_vehicule=".$id_vehicule; //AND mv.date_fin >= '".$aujourdhui."'";
			if($annee_rechercher != date('Y'))
			...
			if($res_maintenance){
				...
				while ($obj_maintenance = $db->fetch_object($res_maintenance)) {
					...
					print "<tr><td style='padding: 10px;'>".$desc."</td><td style='padding: 10px;'>".$type_maint."</td><td style='padding: 10px;'>".$obj_maintenance->date_maintenance."</td>";
					...
					print "<td style='padding: 10px;'>".$obj_maintenance->prochain_kilometrage." km</td><td style='padding: 10px;'>".$outils."</td><td style='padding: 10px;'>".$img."</td></tr>";
