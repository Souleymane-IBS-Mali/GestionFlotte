...
llxHeader("", "Réparations");
//Titre
print load_fiche_titre($langs->trans("Liste des réparations"), '', '');
...
// Recuperation des information après le clique sur l'onglet Salaire au niveau du module user
			...
			$date_maintenance = GETPOST("date_maintenance");
	...
	//Partie affichage du Contrat ------------------------------------------------------------------------------------------------------------------------------------------
		...
		print '<form name="ajouter" method="POST" action="'.$_SERVER["PHP_SELF"].'?mainmenu=gestionflotte&leftmenu=maintenance">';
	...
	print "</td><td>Date réparation<br><input type='date' name='date_maintenance' value='".$date_maintenance."' size='10'></td>";
		/*print "</td><td>Date fin<br><input type='date' name='date_fin' value='".$date_fin."' size='10'></td>";
		...
		//statut
		print "</td><td >";
		print "<input style='padding: 2px;' type='submit' class='button' value='Rechercher' >";
		print "</form><br>";
		print '<a class="button" style="padding: 2px;"" href="'.$_SERVER["PHP_SELF"].'?mainmenu=gestionflotte&leftmenu=maintenance" >Annuler</a>';
		print "</td></td></tr>";
	...
	$sql_maintenance .= " LEFT JOIN ".MAIN_DB_PREFIX."vehicule as vh on mv.fk_vehicule = vh.rowid";
		...
		if(!empty($statut))
			...
			if(!empty($date_fin))
				$sql_maintenance .= ' AND mv.date_fin = "'.$date_fin.'"';
			...
			if($res_maintenance){
				...
				while($obj_mixte = $db->fetch_object($res_maintenance)){
					...
					print "<tr class='fieldrequired'><td><a href='./onglets/maintenance_vehicule.php?mainmenu=gestionflotte&leftmenu=liste_vehicule&id_vehicule=".$obj_mixte->vhrowid."'>".($obj_mixte->libelle?:"N/A")."</a></td>";
						...
						$etat = $actl[1];
						if($obj_mixte->date_fin < $aujourdhui)
							$etat = $actl[0];
						print "<td >".$etat."</a></td>";
						//print "<td></td>";
